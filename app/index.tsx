  ...
  try {
    ...
    // Handle custom scheme: agent://pay?...
      ...
      return {
        ...
        token: null,
        memo: null,
        human: "Invalid URI: must use https://wallet.solana-agent.com/pay or agent:// scheme",
        valid: false,
      };
    }
...
export default function Chat() {
  ...
  }, [pendingDeepLinkParams, user, alreadyDelegated, biometricAuthenticated]);
  ...
  useEffect(() => {
    if (params && typeof params.to === "string") {
      ...
      // Clear the params from the URL so it doesn't re-trigger
      router.replace("/");
    }
  }, [params, router]);
  ...
  useEffect(() => {
    // Only handle NFC if user is authenticated, delegated, AND biometric auth passed
    if (!user || !alreadyDelegated || !biometricAuthenticated) return;
  ...
  }, [user, alreadyDelegated, biometricAuthenticated]);

  // WebSocket connection and auto-reconnect
  useEffect(() => {
    if (!user?.id || !isReady || !alreadyDelegated || !isConnected) {
      if (wsRef.current) {
        wsRef.current.close();
        wsRef.current = null;
      }
    ...
    const connect = async () => {
      if (!user?.id || !isReady || !alreadyDelegated || !isConnected) return;
      const jwt = await getAccessToken();
      if (!jwt) {
      ...
      ws.onclose = () => {
        ...
        setWsStreaming(false);
        wsRef.current = null;
        if (shouldReconnect && isConnected && user?.id && isReady && alreadyDelegated) {
  ...
  }, [user?.id, isReady, getAccessToken, alreadyDelegated, isConnected]);
  ...
  // Authenticated and delegated and biometrics passed
  if (isReady && user && alreadyDelegated && biometricAuthenticated) {
    return (
      <PaperProvider>
